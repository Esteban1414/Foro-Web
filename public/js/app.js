const app = {
  routes: {
    ...
    getObjetiveInfo: "/Categories/getObjetiveInfo",
  ...
  objetiveInfo: function () {
    const modalContainer = $("<div></div>");
    const queryString = window.location.search;
    const urlParams = new URLSearchParams(queryString);
    const category = urlParams.get("category");
    fetch(this.routes.getObjetiveInfo + "?category=" + category)
      .then((res) => res.json())
      .then((cats) => {
        html = this.objetive(cats[0]);
  ...
  <div id="image-modal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
            ...
            <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                ...
                <img src="http://forus.com/resources/assets/img/categories/banners/${cat.banner}" alt="Large Image" class="w-auto h-auto max-w-full max-h-full">
            </div>
          
                <div class="flex flex-col md:flex-row space-y-4 md:space-y-0">
                    <!-- Contenedor de texto a la izquierda -->
                    <div class="w-10/12 p-4">
                      <h1 class="text-black flex justify-center w-full text-3xl font-bold dark:text-white text-center mb-5">${cat.name}</span></h1>
                      <p class=" text-black dark:text-white text-justify mb-5 text-lg font-normal">${cat.description}</p>
                    </div>

                    <!-- Contenedor de imagen y texto a la derecha -->
                    <div class="w-auto p-4 flex flex-col items-center relative top-14">
                        <img class="w-40 h-40 rounded-xl" src="http://forus.com/resources/assets/img/categories/gifs/${cat.gif}" alt="Half Image" class="rounded-lg max-w-full h-auto mt-2">
                        <div class="hashtag text-white font-bold bg-gray-400 dark:bg-gray-500 rounded-full px-4 py-1 text-center h-fit cursor-pointer relative mt-5">
                          <span class="z-10">${cat.name}</span>
                      </div>
                    </div>
                </div>
            </div>
...
`;
  ...
  displayNotifications: function (notif) {
    ...
    container.html("");
    const allNotifications = [...notif.like, ...notif.comment, ...notif.share];
